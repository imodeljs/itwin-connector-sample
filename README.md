...
However in certain cases, where a specific format is not covered, one can develop a new connector using  [iTwin.js SDK](https://www.itwinjs.org/)

![](./imodel_connector_backend.png)

The `@bentley/imodel-bridge` package, provided as part of the iTwin.js SDK makes it easier to write an iTwin connector backend that brings custom data into a digital twin. To run this environment with the iTwin.js library that this package depends on requires JavaScript engine with es2017 support.

Note: Please keep in mind iModelBridge is used as a synonym for iTwin Connector since it bridges the gap between input data and a digital twin.
...
- Generate [changesets](https://www.itwinjs.org/learning/imodelhub/#the-timeline-of-changes-to-an-imodel) and load data into an iModel.
...
As discussed in [Element Fundamentals](https://www.itwinjs.org/bis/intro/element-fundamentals/), the Display Labels are created through the following logic:
...
Rather than starting over when the source data changes, a connector should be able to detect and convert only the changes. That makes for compact, meaningful changesets, which are added to the iModel's [timeline](https://www.itwinjs.org/learning/imodelhub/#the-timeline-of-changes-to-an-imodel).
...
For COBie data, this sample connector uses a combination of sheet name and unique row name to map data into the iModel. See [updateElementClass](./COBie-connector/src/DataAligner.ts) function in the provided sample. When the identifier is provided to the synchronizer, it is stored inside the ExternalSourceAspect class, in the Identifier property.
...
In case of COBie connector, the above algorithm implemented inside the [align method of DataAligner](./COBie-connector/src/DataAligner.ts)
...
- BridgeRunner: [Opens a local briefcase copy](https://www.itwinjs.org/learning/backend/imodeldb/) of the iModel that is to be updated.
- Import or Update Schema
  - Connector: Possibly [import an appropriate BIS schema into the briefcase](https://www.itwinjs.org/learning/backend/schemasandelementsintypescript#importing-the-schema)  or upgrade an existing schema.
  - BridgeRunner: [Push](https://www.itwinjs.org/learning/backend/imodeldbreadwrite/#pushing-changes-to-imodelhub) the results to iModelHub.
- Convert Changed Data
  ...
  - BridgeRunner: Obtain required [Locks and Codes](https://www.itwinjs.org/learning/backend/concurrencycontrol/) from the iModelHub and/or code server.
- BridgeRunner: [Push](https://www.itwinjs.org/learning/backend/imodeldbreadwrite/#pushing-changes-to-imodelhub) changes to the iModelHub.
...
- [Importing a schema and bootstrapping definitions](https://www.itwinjs.org/learning/backend/schemasandelementsintypescript#importing-the-schema)
- [AccessToken](https://www.itwinjs.org/learning/common/accesstoken/)
- [BriefcaseManager.create](https://www.itwinjs.org/reference/imodeljs-backend/imodels/briefcasemanager/createstatic)
- [BriefcaseDb.open](https://www.itwinjs.org/reference/imodeljs-backend/imodels/briefcasedb/openstatic)
- [IModelDb.saveChanges](https://www.itwinjs.org/reference/imodeljs-backend/imodels/imodeldb/savechanges)
- [BriefcaseDb.pullAndMergeChanges](https://www.itwinjs.org/reference/imodeljs-backend/imodels/briefcasedb/pullandmergechanges)
- [BriefcaseDb.pushChanges](https://www.itwinjs.org/reference/imodeljs-backend/imodels/briefcasedb/pushchanges)
- [ConcurrencyControl](https://github.com/imodeljs/imodeljs/tree/master/docs/learning/backend/ConcurrencyControl.md)
- [DefinitionModel.insert](https://www.itwinjs.org/reference/imodeljs-backend/models/definitionmodel/insertstatic/)
- [PhysicalModel.insert](https://www.itwinjs.org/reference/imodeljs-backend/models/physicalmodel/insertstatic)
- [Insert a Subject element](https://www.itwinjs.org/learning/backend/createelements/#subject)
- [Insert a ModelSelector element](https://www.itwinjs.org/learning/backend/createelements/#modelselector)
- [Insert a CategorySelector element](https://www.itwinjs.org/learning/backend/createelements/#categoryselector)
- [Insert a DisplayStyle3d element](https://www.itwinjs.org/learning/backend/createelements/#displaystyle3d)
- [Insert a OrthographicViewDefinition element](https://www.itwinjs.org/learning/backend/createelements/#orthographicviewdefinition)
- [Logging](https://www.itwinjs.org/learning/common/logging/)
